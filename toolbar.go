...
func (app *Config) addHoldingsDialog() dialog.Dialog {
	...
	addForm := dialog.NewForm(
		...
		[]*widget.FormItem{
			...
			{Text: "Purchaseprice", Widget: purchasePriceEntry},
			{Text: "Purchasedate", Widget: purchaseDateEntry},
		},
		func(valid bool) {
			if valid {
				amount, _ := strconv.Atoi(addAmountEntry.Text)
				purchaseDate, _ := time.Parse("01-02-2006", purchaseDateEntry.Text)
